--- Taken from   https://github.com/soumith/dcgan.torch/blob/master/
...
local initcheck = argcheck{
   ...
   {name="split",
    type="number",
    help="Percentage of split to go to Validation"
   },
   ...
   {name="forceClasses",
    ...
    .. "class indices (valLoader/testLoader for example)",
    opt = true},

   {name="sampleHookVal",
    type="function",
    help="applied to sample during validation(ex: for lighting jitter). "
...
function dataset:__init(...)
   ...
   if not self.sampleHookVal then self.sampleHookVal = self.defaultSampleHook end
   if not self.sampleHookTest then self.sampleHookTest = self.defaultSampleHook end

   -- find class names

                                                              -- Check the order in training donkey.lua file.


  function inTable(tbl, item)
      for key, value in pairs(tbl) do        
          if value == item then -- print('value',value)
           return true 
         end
      end
      return false
   ...
   self.classes = {}
   local classPaths = {}
   if self.forceClasses[1] then
    local counter=1
      for k,v in pairs(self.forceClasses[1]) do
         if inTable(self.forceClasses[2],v)  then 
           self.classes[counter] = v
           classPaths[counter] = {}
           counter=counter+1
         end
      end
...
print(' privious #self.classes',#self.classes)
   ...
   for k,path in ipairs(self.paths) do
      ...
      for k,dirpath in ipairs(dirs) do
         ...
         if inTable(self.forceClasses[2],class)  then 
...
print('#self.classes',#self.classes)
  ...
  if ffi.os == 'OSX' then
   ...
   if self.split == 100 then
      ...
      print('Splitting validation and test sets to a ratio of '
               .. self.split .. '/' .. (100-self.split))
      self.classListVal = {}
      self.classListTest  = {}
      self.classListSample = self.classListVal
      local totalTestSamples = 0
      -- split the classList into classListVal and classListTest
      for i=1,#self.classes do
         ...
         self.classListVal[i] = torch.LongTensor(splitidx)
         for j=1,splitidx do
            self.classListVal[i][j] = list[perm[j]]
         end
         if splitidx == count then -- all samples were allocated to val set
...
function dataset:getByClass(class)
   ...
   return self:sampleHookVal(imgpath)
end
...
-- sampler, samples from the validation set.
